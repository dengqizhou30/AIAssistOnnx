...
Ort::Value YOLOv6::transform(const cv::Mat& mat_rs)
{
    cv::Mat canvas;
    //cv::cvtColor(mat_rs, canvas, cv::COLOR_BGR2RGB);
    //canvas.convertTo(canvas, CV_32FC3, 1.f / 255.f, 0.f);
    mat_rs.convertTo(canvas, CV_32FC3, 1.f / 255.f, 0.f);
    // (1,3,640,640) 1xCXHXW
    return onnxlib::create_tensor(
        canvas, input_node_dims, memory_info_handler,
        input_values_handler, onnxlib::CHW);
}

// letterbox
void YOLOv6::resize_unscale(const cv::Mat& mat, cv::Mat& mat_rs,
...
void YOLOv6::detect(const cv::Mat& mat, std::vector<lite::types::Boxf>& detected_boxes,
    ...
    unsigned int nms_type)
{
    ...
    // 3. rescale & exclude.
    std::vector<lite::types::Boxf> bbox_collection;
...
void YOLOv6::generate_bboxes(const YOLOv6ScaleParams& scale_params,
    std::vector<lite::types::Boxf>& bbox_collection,
    ...
    int img_width)
{
    ...
    for (unsigned int i = 0; i < num_anchors; ++i)
    {
        ...
        //80个类别中，第一个类别为人。程序只需要识别人员，所以无需循环判断其他类别的置信度
        /*
...
void YOLOv6::nms(std::vector<lite::types::Boxf>& input, std::vector<lite::types::Boxf>& output,
